...
export function TicketSuccessState({ email, description, onSubmitAnother }: TicketSuccessStateProps) {
  return (
    <div className="mx-auto flex max-w-3xl flex-col items-center rounded-3xl border border-[var(--palette-stone)] bg-[var(--palette-warm-white)] p-6 text-center shadow-soft md:p-16">
      ...
      <p className="mt-3 text-lg leading-7 text-[var(--palette-dark-olive)]">A confirmation email has been sent to <span className="font-bold text-[var(--palette-dark-olive)]">{email}</span>.</p>
      ...
      <div className="mt-8 flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
        ...
        <button className="rounded-lg bg-[var(--palette-off-white)] px-6 py-3 text-sm font-bold text-[var(--palette-dark-olive)] transition-colors hover:bg-[var(--palette-stone)]" onClick={onSubmitAnother} type="button">Submit Another</button>
